...
export default function BidOffer() {
  ...
  const [auctionData, setAuctionData] = useState<Auction[]>(
    Array.from({ length: 50 }, (_, i) => ({
      ...
      hasOffer: i % 3 === 0 // Show offer button for every 3rd item
    }))
  );
 ...
 const handleView = (auctionId: number): void => {
    router.push(`/dashboard/auction/bid-offer/view?auctionId=${auctionId}`);
    // Add your offer logic here
};

  const handleOffer = (auctionId: number): void => {
    router.push(`/dashboard/auction/bid-offer/${auctionId}`);
  };
    ...
    <div className='min-h-screen '>
      <div className='md:ml-5 p-6 flex flex-col gap-6'>
        ...
        <div className='flex bg-white p-5 py-8 flex-wrap items-center gap-5'>
          {/* Search Input */}
          <div className='relative flex-1 min-w-[200px] max-w-[700px]'>
            <svg 
                  ...
                  <tr key={auction.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex gap-2">
                        {auction.hasOffer && (
                          <button
                            onClick={() => handleOffer(auction.id)}
                            className="px-4 py-1.5 bg-[#FFFCEB] border-2 border-[#FFE135] text-gray-900 text-sm rounded hover:bg-[#FFE135] transition-colors"
                          >
                            Offer
                          </button>
                        )}
